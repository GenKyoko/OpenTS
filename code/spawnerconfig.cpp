...
void SpawnerConfig::Read_INI(CCINIClass& spawn_ini)
{
	...
	spawn_ini.Get_String(SETTINGS, "SaveGameName", SaveGameName);
	...
	spawn_ini.Get_String(SETTINGS, "Scenario", ScenarioName);
	spawn_ini.Get_String(SETTINGS, "MapHash", MapHash);
	spawn_ini.Get_String(SETTINGS, "UIMapName", MapName);
	...
	spawn_ini.Get_String(TUNNEL, "Ip", TunnelIp);
	...
	spawn_ini.Get_String(SETTINGS, "CustomLoadScreen", CustomLoadScreen);
	...
	spawn_ini.Get_String(SETTINGS, "DifficultyName", DifficultyName);
	...
	for (int i = 0; i < std::size(GlobalFlags); i++) {
		...
		if (GlobalFlags[i] > 0) {
			DebugString("[Spawner] Read global %d as %d from %s\n", i, GlobalFlags[i], buffer);
		}
	}
}
...
void SpawnerConfig::PlayerConfig::Read_Player_INI(CCINIClass& spawn_ini, int index)
{
	...
	if (spawn_ini.Is_Present(SECTION)) {
		IsHuman = true;
		Difficulty = -1;
		spawn_ini.Get_String(SECTION, "Name", Name);
		Color = static_cast<int>(spawn_ini.Get_Int(SECTION, "Color", Color));
		House = static_cast<HousesType>(spawn_ini.Get_Int(SECTION, "Side", House));
		spawn_ini.Get_String(SECTION, "Ip", Ip);
		Port = spawn_ini.Get_Int(SECTION, "Port", Port);
	}
	else if (!IsHuman) {
		Color = static_cast<int>(spawn_ini.Get_Int("HouseColors", MULTI_TAG, Color));
