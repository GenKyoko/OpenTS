 ...
 *  @author: CCHyper
 */
int CnCNet5UDPInterfaceClass::Message_Handler(HWND hWnd, UINT uMsg, UINT wParam, LONG lParam)
{
	sockaddr_in addr;
	int rc;
	int addr_len;
	...
	case FD_READ:
		...
		if (rc == SOCKET_ERROR) {
			DebugString("CnCNet5: Receive_From returned %d!\n", WSAGetLastError());
			Clear_Socket_Error(Socket);
			return 0;
		}

		if (rc < static_cast<int>(sizeof(unsigned int)) || rc > static_cast<int>(772)) {
			DebugString("CnCNet5: Throwing away packet with invalid length %d!\n", rc);
			return 0;
		}
		...
		if (!address_mapped) {
			DebugString("CnCNet5: Throwing away packet from an unknown endpoint.\n");
			return 0;
		}
		...
		if (rc != 0) {
			 ...
			 */
			for (int i = 0; i < Get_Num_Local_Addresses(); ++i) {
			...
			packet->CRC = *reinterpret_cast<unsigned int*>(ReceiveBuffer);
			std::memcpy(packet->Buffer, ReceiveBuffer + sizeof(packet->CRC), rc - sizeof(packet->CRC));
			if (!Passes_CRC_Check(packet)) {
				DebugString("CnCNet5: Throwing away malformed packet!\n");
	...
	case FD_WRITE:
		...
		if (i >= std::size(AddressList) || i < 0) {
			DebugString("CnCNet5: Dropping outgoing packet with invalid destination index %d.\n", i);
			OutBuffers.Delete_Index(packetnum);
		 ...
		 */
		rc = Send_To(Socket, reinterpret_cast<const char*>(&packet->CRC), packet->BufferLen + sizeof(packet->CRC), 0, &addr, sizeof(addr));
		if (rc == SOCKET_ERROR) {
			if (WSAGetLastError() == WSAEWOULDBLOCK) {
				return 0;
			}

			DebugString("CnCNet5: Dropping outgoing packet after Send_To failed with error %d.\n", WSAGetLastError());
			Clear_Socket_Error(Socket);
			OutBuffers.Delete_Index(packetnum);
			Delete_Out_Buffer(packet);
			return 0;
...
int CnCNet5UDPInterfaceClass::Send_To(SOCKET s, const char* buf, int len, int flags, sockaddr_in* dest_addr, int addrlen)
{
	 ...
	 */
	if (TunnelPort == 0) {
		DebugString("CnCNet5: TunnelPort is invalid in Send_To!\n");
...
int CnCNet5UDPInterfaceClass::Receive_From(SOCKET s, char* buf, int len, int flags, sockaddr_in* src_addr, int* addrlen)
{
	 ...
	 */
	if (TunnelPort == 0) {
		DebugString("CnCNet5: TunnelPort is invalid in Recieve_From!\n");
	...
	if (ret < 5 || *bufto != TunnelID) {
		DebugString("CnCNet5: recvfrom returned invalid data!\n");
		return -1;
	}
