...
void LatencyLevel::Apply(LatencyLevelEnum new_latency_level)
{
	...
	Session.Messages.Add_Message(nullptr,
		0,
		Get_Latency_Message(new_latency_level),
		Fetch_Scheme_Index_By_Name("White"),
		TextPrintType(TPF_USE_GRAD_PAL | TPF_FULLSHADOW | TPF_6PT_GRAD),
		static_cast<int>(Rule->MessageDelay * TICKS_PER_MINUTE / 2)
	);
}
